    ...
    <tiles:putAttribute name="body">
    	...
    	<div class="breadcrumbs">
        	...
        	<div class="col-md-12 md-margin-bottom-40">
            	<h1 class="pull-left">받은 쪽지</h1>
            	<ul class="pull-right breadcrumb">
                	<li><a href="index">Home</a></li>
                	<li class="active">받은 쪽지</li>
            	</ul>
            	</div>
        	</div><!--/container-->
    	</div><!--/breadcrumbs-->
		...
		<div class="col-md-9">
                   ...
                   <table class="table">
                        ...
                        <c:forEach begin="${pageInfo.startRow}" end="${pageInfo.endRow}" var="message" items="${messageList}">
                        	<tr>
                        		<td>${message.num }</td>
                        		<td><a href = "messageDetail?message_num=${message.message_num}">${message.content }</a></td>
                        		<td>${message.sender }</td>
                        		<td>${message.write_date }</td>
                        		<c:choose>
                				<c:when test="${sessionScope.member != null && sessionScope.member.id == 'admin'}">
                        		<td>
									<button class="btn btn-danger btn-xs" onclick="location.href='deleteMessage?message_num='+${message.message_num}">
